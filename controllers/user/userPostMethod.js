...
async function signUp(req, res) {
	try {
		...
		if (req.query.referral_id || req.query.referral_id != undefined) {
			referralCode = couponGenerator.getCouponFromReferralLink(
				req.query.referral_id
			);
			...
			referralUser.WalletBalance =
				Number(referralUser.WalletBalance) + Number(reward.ReferalReward);
...
async function signin(req, res) {
	try {
		...
		if (user.Disable == true) {
			return res.status(400).json({
				message:
					"Your account has been blocked, Kindly contact the support team for further details",
			});
		}
