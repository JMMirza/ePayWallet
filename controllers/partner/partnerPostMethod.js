...
async function signUp(req, res) {
    try {
        ...
        if (req.query.referral_id || req.query.referral_id != undefined) {
            ...
            const referralUser = await Partner.findOne({
                Coupon: referralCode
            })
            if (!referralUser) {
...
async function sendMoney(req, res) {
    ...
    const reciever = await Partner.findOne({
        PhoneNumber: req.body.phoneNum
    })
    const sender = await Partner.findOne({
...
async function addLocation(req, res) {
    try {
        const user = await Partner.findById(req.auth._id)
        if (!user) {
            return res.status(400).json({
                message: "User not found"
            })
        }
        const location = await PartnerLocation.findOne({
            partnerId: req.auth._id
        })
        if (location) {
            return res.status(400).send({
        ...
        const store = await PartnerLocation.create({
            partnerId: req.auth._id,
